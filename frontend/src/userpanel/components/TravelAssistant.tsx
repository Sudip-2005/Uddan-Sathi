...
const TravelAssistant = () => {
  ...
  const [messages, setMessages] = useState([
    { role: "model", text: "### Welcome to UdaanSathi AI! 🤖\nHow can I help you today?" }
  ]);
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[9999] font-sans">
      {isOpen && (
        <div className="absolute bottom-20 right-0 w-[400px] h-[600px] bg-white dark:bg-gray-900 rounded-[2rem] shadow-2xl border border-slate-200 dark:border-gray-700 flex flex-col overflow-hidden animate-in slide-in-from-bottom-5">
          ...
          <div className="p-5 bg-gradient-to-r from-blue-600 to-blue-700 text-white flex justify-between items-center">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-bold leading-none text-base">UdaanSathi AI</p>
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-5 bg-[#F8FAFC] dark:bg-gray-800">
            {messages.map((msg, i) => (
              <div key={i} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
                <div className={`max-w-[85%] px-4 py-3 rounded-2xl text-sm shadow-sm leading-relaxed ${
                  msg.role === "user" 
                    ? "bg-blue-600 text-white rounded-tr-none" 
                    : "bg-white dark:bg-gray-700 text-slate-800 dark:text-gray-100 border border-slate-100 dark:border-gray-600 rounded-tl-none prose prose-slate dark:prose-invert"
                }`}>
                  {msg.role === "model" ? (
          ...
          {/* Input Area */}
          <div className="p-4 bg-white dark:bg-gray-900 border-t border-slate-100 dark:border-gray-700 flex gap-2 items-center">
            <input 
              value={input}
              onChange={(e) => setInput(e.target.value)}
              onKeyDown={(e) => e.key === "Enter" && handleSend()}
              placeholder="Ask about your query..."
              className="flex-1 bg-slate-50 dark:bg-gray-800 border border-slate-200 dark:border-gray-600 rounded-xl px-4 py-3 text-sm text-slate-900 dark:text-white placeholder-slate-400 focus:ring-2 focus:ring-blue-600 outline-none transition-all"
            />
            <Button 
              onClick={handleSend} 
